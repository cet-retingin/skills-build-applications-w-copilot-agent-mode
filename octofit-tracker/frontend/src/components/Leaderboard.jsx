...
function Leaderboard() {
  ...
  if (error) return <div className="alert alert-warning" role="alert">Failed to load leaderboard: {error}</div>;
        ...
        {items.length === 0 ? (
          ...
          <ul className="list-group list-group-flush">
            {items.map((item, index) => (
              <li className="list-group-item px-0" key={`leaderboard-${index}`}>
